...
if ( $args < 1 || $1 == '--help' || $1 == '-h' ) then
        ...
        printf "${KBold}Despcription: ${KDefault}This program is used to change some parameters of a star file so that it can be used for the shrinked particels.\n"
        ...
        printf "${KBold}example:${KDefault} $Proc_name 4 r3d2_cls2_data.star  \n"
        printf "${KBold}example:${KDefault} $Proc_name 2 r3d2_cls*_data.star  \n"
...
set root=`echo $starf |sed 's/.star//g' `

set starf_new=${root}_shrink${shrink}.star
...
gawk '$'$rlnDetectorPixelSizeIndex'~/[0-9]/ { for(i=1;i<=NF;i++){if (i=='$rlnDetectorPixelSizeIndex') printf("%f  ",$i * '$shrink' ) ;else if (i=='$rlnImageNameIndex') printf("%s_shrink%d.mrcs  ",substr($i,0,length($i)-5),'$shrink' );  else    printf("%s  ", $i); }  printf("\n");}' $starf >>  $starf_new

else if ( ( ! ($rlnDetectorPixelSizeIndex == "" || $rlnImageNameIndex == "") ) && ( ! ( $rlnOriginXIndex == "" || $rlnOriginYIndex == "") ) ) then

gawk '$'$rlnOriginXIndex'~/[0-9]/ && $'$rlnOriginYIndex'~/[0-9]/ && $'$rlnDetectorPixelSizeIndex'~/[0-9]/ { for(i=1;i<=NF;i++){ if( i=='$rlnOriginXIndex' || i=='$rlnOriginYIndex' ) printf("%f  ",$i / '$shrink' ); else if (i=='$rlnDetectorPixelSizeIndex') printf("%f  ",$i * '$shrink' ) ;else if (i=='$rlnImageNameIndex') printf("%s_shrink%d.mrcs  ",substr($i,0,length($i)-5),'$shrink' );  else    printf("%s  ", $i); }  printf("\n");}' $starf >>  $starf_new
